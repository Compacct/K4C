...
@Component({
  ...
  styleUrls: ['./outlet-sale-bill-without-batch-select.component.css'],
...
autoaFranchiseBill() {
 ...
 add(valid) {
   ...
   if (this.gststatus == "NO GST") {
     ...
     var Amount = Number(BQ * Number(this.ObjaddbillForm.Sale_rate));
    ...
    // var tax = Number(Number(rate) * Number(this.ObjaddbillForm.Stock_Qty));
     ...
     var IGST_Per = 0;
     var IGST_Amount = 0 ;
     totalgst = Number(IGST_Amount) ? Number(SGST_Amount) + Number(CGST_Amount) + Number(IGST_Amount) : Number(SGST_Amount) + Number(CGST_Amount);
 ...
 GetSelectedBatchqty () {
   const sameproductwithbatch = this.productSubmit.filter(item=>item.Product_ID === this.ObjaddbillForm.Product_ID );
   ...
   //   // }
   // }
   var TotalBatch = undefined;
   let count = 0;
   this.Batch_NO.forEach(i => {
     count = count + Number(i.Qty);
   });
   TotalBatch = (count).toFixed(2);
     ...
     if(!quantityAddChckFlag){
       this.compacctToast.clear();
         this.compacctToast.add({
           ...
           detail: "Available batch quantity " + ': ' + TotalBatch
         });
     }
 ...
 delete(id) {
   let temarr = [];
   for(let i = 0; i < this.productSubmit.length; i++){
     if (this.productSubmit[i].Product_ID.toString() !== id.toString()) {
       temarr.push(this.productSubmit[i]);
     }
 }
 this.productSubmit = [...temarr];
 ...
 CalculateDiscount(){
  if (this.ObjcashForm.Credit_To_Amount){
    ...
    this.productSubmit.forEach(el=>{ 
      ...
      // netamount = Number((Number(el.Taxable) - Number(el.Dis_Amount)) + Number(sgstperamt) + Number(cgstperamt));
      //this.Dis_Amount = undefined;
    ...
    this.productSubmit.forEach(el=>{
      ...
      //el.Taxable = Number((Number(el.Amount) - Number(el.Dis_Amount)) - Number(totalgstamt)).toFixed(2);
